...
lm.mod <- function(){
  ...
  for(i in 1:N){
    # each y_i is a realization of a normal distribution with mean mu_i and variance sig
  ...
  # Priors:
  alpha ~ dnorm(0, .01) # precision .01 = variance 100
  b1 ~ dnorm(0, .01)
  b2 ~ dnorm(0, .01)
  
  sig.sq ~ dgamma(1, .01)
  sigma <- sqrt(sig.sq) # Because we simulated the data with standard deviation, that's what we want to look at
                        # All this is doing is giving us a transformed variabl for convenience.
...
initial.values.lm <- replicate(3, list(list(
  alpha = rnorm(1, 0, 30), b1 = rnorm(1, 0, 30), b2 = rnorm(1, 0, 30), sig.sq = rgamma(1, 1, .1)
  )))
```
...
mixed.mod <- function(){
  # Likelihood
  for(i in 1:N){
    # in this first line, we descibe each observation y_i as a realization of a normal process 
    y[i] ~ dnorm(mu[i], 1/sig.obs^2)
    # here we define the observation-specific mean as a linear function of the site-specific mean
  ...
  for(j in 1:n.sites){# Now we're declaring each site-level intercept
    alpha[j] ~ dnorm(alpha0, 1/sig.site^2) # each site level intercept is a realization of a normal process
  }
...
initial.values.lmm <- replicate(3, list(list(
  ...
  b1 = rnorm(1, 0, 30),
  b2 = rnorm(1, 0, 30),
  sig.obs = rgamma(1, 1, .1),
  sig.site = rgamma(1, 1, .1))))

# Parameters to save as output. If desired, you can save each site-level mean also, but I do not do so here.
...
```{r JAGS LMM results}
sapply(1:ncol(fm.mixed.dat), function(x) plotPost(fm.mixed.dat[,x], main = colnames(fm.mixed.dat)[x]))
```

# Try it on your own!

Here we'll use the crab burrow data from the test last week. That is, number of burrows as a function of salinity. This is count data, so it could be modeled with a Poisson or negative-binomial. For simplicity, here, we won't worry about which is better and just use a Poisson. Recall that in Poisson regression, we use the log link:
$$ Poisson(Y_i|\lambda_i) \\
  \lambda_i = e^{\alpha + \beta_1 X_1}$$
  
*Question*: Draw the Bayesian network of this model, and write out the mathematical probability statement. For priors, use gamma(1, .1) on mean abundance, normal(mean = 0, variance = 10) on your salinity effect.
```{r Try it - data}
...
*Question* Interpret these results. What is the mean number of burrows and its confidence intervals? What is the effect of salinity and its confidence intervals? Given our data and assumptions, what is the probability that salinity has no effect (approximately)?

Next, we'll try a hierarchical (random-intercept) model.

These are data from my masters thesis, in which I studied prevalence of the tick-borne pathogen *Ehrlichia chaffeensis* across different landscapes. I have simplified the dataset by consolidating EC occurrence across years, and omitting heterogeneity in tick abundance. I have also only included three varaibles: proportion of forest within the surrounding 100 ha, length of forest edge within that 100 ha, and the proportion of that forest that is evergreen/coniferous. Notice that there are plots nested within sites, and so we need to account for potential covariance of plots within sites due to characteristics not modeled.

In this version of the dataset, EC occurrence is recorded as presence-absence, so this will need to be a binomial/logistic regression. The model we want to write will predict the presence of EC as a function of forest cover, forest edge, and coniferous forest cover. Hypotheses being that forest size, fragmentation, and 'quality' will affect vertebrate host density.

```{r EC data}
# FYI: Data are centered/scaled
read.csv("Ehrlichia_occurrence.csv")
```

*Question*: Draw the Bayesian network and write the probability statement for a logistic regression model with 3 predictors and a random intercept. Hint: in a logistic regression, our observations are considered the realization of a Bernoulli process. Recall that the form for logistic regression is logit(p) = b0 + BX, where p is the probability of the event, and logit(p) is the log of the odds ratio (i.e. successes/failures). Because your variables are on the logit scale, you need to be careful with their priors. Here, give the grand mean a normal prior with mean = 0 and variance = 2, betas a normal prior with mean = 0 and variacne = 5, and the among site variance a gamma prior with parameters 1, 1.
...
*Question* Interpret these results.
